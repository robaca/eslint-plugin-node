...
module.exports = {
    ...
    create(context) {
        ...
        const tryExtension = getTryExtensions(context)
        ...
        function verify({ filePath, name, node }) {
            ...
            console.log("Resolved ext", resolvedExt, filePath)
            if (!resolvedExt && existingExts.length !== 1) {
                // Ignore if the file extension could not be determined one.
                return
            }
            const foundExt = resolvedExt || existingExts[0]
            const ext =
                esm && (foundExt === ".ts" || foundExt === ".tsx")
                    ? ".js"
                    : foundExt
